...
description: 从 1688 国际站（阿里巴巴国际站 alibaba.com）提取类目名、商品标题、商品链接等语料，候选关键词去重后追加进 FB/X 词库，并在收尾前抽取 5% 新词用真实浏览器在 X 上验证帖量；同时所有看到的类目与商品语料写入 SQLite 语料表。当用户说"挖词"、"提取关键词"、"从 1688 找关键词"、"扩词库"时使用。
---
...
**2026-08-23 用户拍板：恢复 X 抽样验证**。此前（2026-08-22）曾拍板不做人工验证、全靠日落机制淘汰，但实际跑下来发现词帖命中率偏低，改为：**每批词追加后、收尾前必须抽 5% 在 X 上用真实浏览器验证帖量**（见第 5 步），抽查不合格的词当场移出词库，其余仍交给日落机制慢慢淘汰。
...
1. **提醒用户登录 1688 国际站（alibaba.com）和 X**（第 5 步抽样验证需要 X 登录态）。
...
## 第 5 步：X 抽样验证（5%，收尾前必做，2026-08-23 用户拍板）

- **抽样**：从本轮新追加的词中随机抽 5%（追加 2000 个就抽 100 个，随机种子固定以便复现）。
- **验证方式：一律用 WebBridge 在用户真实浏览器（带 X 登录态）逐词验证**（2026-08-23 用户明确，禁止用 API 批量刺探代替）：navigate `https://x.com/` 首页 → 找搜索框（`data-testid="SearchBox_Search_Input"`）→ 输入 **`+86 <词>`**（与线上 `x_query()` 同口径，裸词搜不到卖家帖，2026-08-23 用户强调）→ **CDP `Input.insertText` 打字 + ArrowDown+Enter 提交**（合成 fill 触发不了联想、合成回车不生效、typeahead 常无「Search for」选项，此路径 2026-08-23 实测打通）→ 进结果页后 click 切 `Latest` 标签 → `evaluate` 统计 `article` 数与帖正文手机号命中。之后留在结果页复用顶部搜索框验证下一词，不用每次回首页。
- **频率控制**：搜索间隔 5~15 秒随机，X 单站连续操作 ≤15 分钟（约 25~30 词）停手，隔几分钟再开下一个窗口；遇验证码/限流立即停手告知用户。
- **判据**：0 帖 = 死词，当场用 `grep -vFx` 从 `.cache/x_keywords_all.txt` 与 `.cache/fb_keywords_extra.txt` 逐行删除（禁止 `sort -u` 重写）；≥1 帖保留，交给日落机制继续观察。
- **汇报**：抽样数、有帖率、帖量分布、删除词清单；有帖率 < 50% 时明确提示该批词源质量差、下批应调整词源。
