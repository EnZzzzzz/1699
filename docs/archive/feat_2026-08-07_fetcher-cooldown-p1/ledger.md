...
- Step 3.2: complete（文档同步 commit 9096948；终审 commits 3847934..9096948 通过）
  - 终审结论：时长公式逐处零偏差（Sleep 系同源复用 human_pause_duration，比复刻更稳）；非目标零越界（仅 5 个代码文件）；契约三层咬合；测试 18 用例 + 冒烟证据充分
  - 终审修复（主 Agent 账目补登）：SPEC §6 补 E 段用户裁定记录；PLAN Step 3.2 勾选
  - 终审分诊（合并后跟进）：✓ 策略完成日志时序（先打 ✓ 再冷却，中断时会打出 ✓，旧路径不打）——建议尽早修，避免中断场景日志误导排查
  - 终审分诊（不用修，带 ruling）：cooldown=0.0 跳过等待（语义正确）；中间态日志已自洽；测试容差/抖动（破坏证据兜底）
